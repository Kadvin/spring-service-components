...
public abstract class BaseCommand implements Command {
  ...
  protected Properties environmentProperties() {
    ...
    try {
      ...
      // support value with ${variable}
        Enumeration<?> en = props.propertyNames();
        while (en.hasMoreElements()) {
          String key = (String) en.nextElement();
          String value = props.getProperty(key);
          String newValue = interpolate(value, props);
  ...
  protected MigrationLoader getMigrationLoader() {
    return new FileAndJarMigrationLoader(paths.getScriptPath(), paths.getApplicationPath(),
  ...
  protected DatabaseOperationOption getDatabaseOperationOption() {
    ...
    public String interpolate(String origin, Properties props) {
        ...
        while (m.find()) {
            ...
            if (replacement == null) {
                System.err.println("Can't interpolate variable ${" + variable + "} " );
                continue;
            }
            //解析出来的变量可能还需要再解析
